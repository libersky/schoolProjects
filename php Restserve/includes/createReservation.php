<?php
    ...
    if(isset($_POST['reservationSubmit'])&&isset($_POST['restaurantId'])&&isset($_POST['timeTo'])&&isset($_POST['timeFrom'])&&isset($_POST['day'])){

        $restaurantId = htmlspecialchars($_POST['restaurantId']);
        $chosenTimeTo = htmlspecialchars($_POST['timeTo']);
        $chosenTimeFrom = htmlspecialchars($_POST['timeFrom']);
        $chosenDay = htmlspecialchars($_POST['day']);
        $userEmail = htmlspecialchars($_POST['email']);
        $userPhone = htmlspecialchars($_POST['phone']);
        $userName = htmlspecialchars($_POST['name']);
        $chosenTableId = htmlspecialchars($_POST['chosenTableId']);
        ...
        try{
            ...
            error_log($e->getMessage());
            header('Location: ../restaurant.php?error=dbresecrash');
            die();
        }
        ...
        foreach ($reservations as $reservation) {
            if ($chosenDay == $reservation['reservation_date']) {
                ...
                if (!((($startChosenTime[0] <= $startTime[0]) && ($endChosenTime[0] <= $startTime[0])) || ($startChosenTime[0] >= $endTime[0]))) {
                    if($reservation['reservation_table_id']===$chosenTableId){
                        header("Location: ../index.php?error=mistoOsazeno-".$reservation['reservation_table_id']."=".$chosenTableId);
                        die();
                    }
                }
            }
        ...
        $patternName = '/^\p{Lu}\p{Ll}+(\s\p{Lu}\p{Ll}+)+/u';
        if((strlen($userName)<=50)&&(strlen($userName)>=2)&&(preg_match_all($patternName, $userName))){

            $patternEmail = '/\w+\.?\w+@\w+\.[a-z][a-z]+/';
            if((strlen($userEmail)<=50)&&(strlen($userEmail)>=5)&&(preg_match_all($patternEmail, $userEmail))){

                $patternPhone = '/(^[+]420\s?)?\d{3}\s?\d{3}\s?\d{3}$/';
                if((strlen($userPhone)<=16)&&(preg_match_all($patternPhone, $userPhone))){

                    try{
                        $userIdent = '';
                        if(strlen($_SESSION['user_id'])>1){
                            $userIdent = htmlspecialchars($_SESSION['user_id']);
                        }else{
                            $userIdent = 0;
                        }

                        $stmt = $db->prepare('INSERT INTO rest_reservation (reservation_table_id, reservation_user_id, reservation_user_name, reservation_user_email, reservation_user_phone, reservation_date, reservation_time_from, reservation_time_to) 
                                                        VALUES ( :tableId, :userId, :userName, :userEmail, :userPhone, :date, :timefrom, :timeto);');
                        $stmt->execute([
                            ...
                            ':userPhone' => $userPhone,
                            ':date' => $chosenDay,
                            ':timefrom' => $chosenTimeFrom,
                            ':timeto' => $chosenTimeTo
                        ]);

                        if(isset($_SESSION['user_id'])){
                            header('Location: ../board.php?success=reservationmade');
                            die();
                        }
                        else{
                            header('Location: ../restaurant.php?success=reservationmade&'.$history);
                            die();
                        }
                    ...
                    header('Location: ../index.php?error=dbinserfailed');
                }
                else{
                    header('Location: ../index.php?error=incorectphone');
                }
            }
            else{
            ...
            header('Location: ../index.php?error=incorrectname');
        }
    }
    else{
        header("Location: ../index.php?error=fatalerrormodifikovananemodifikovatelnapole");
    }
